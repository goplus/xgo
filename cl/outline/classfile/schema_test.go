...
func TestLoadResourceSchema(t *testing.T) {
	t.Run("MissingFrameworkPackagePath", func(t *testing.T) {
		_, err := LoadResourceSchema(&xast.Package{}, &modfile.Project{}, &outline.Config{})
		if err == nil || err.Error() != "project has no framework package path" {
			t.Fatalf("expected missing framework package path error, got %v", err)
		}
	})
...
// SetCostume sets one sprite's current costume.
//
//xgo:class:resource-api-scope-binding param.1 param.0
func SetCostume(sprite SpriteHandle, costume SpriteCostumeName) {}
`,
		})

		schema, err := loadTestResourceSchema(pkg, conf)
		if err != nil {
		...
		spriteHandle, ok := schema.Package.Scope().Lookup("SpriteHandle").(*types.TypeName)
		if !ok {
			t.Fatal("SpriteHandle type not found")
		}
		if got, ok := schema.HandleKindOfType(spriteHandle.Type()); !ok || got != sprite {
			t.Fatal("HandleKindOfType did not follow alias chain to sprite handle")
		}
		...
		apiScopeBindings[0].TargetParam = 1
		apiScopeBindings = schema.APIScopeBindings(setCostume)
		if apiScopeBindings[0].TargetParam != 0 {
			t.Fatal("APIScopeBindings returned schema-owned storage")
		}
		...
		setCostumeFn, ok := schema.Package.Scope().Lookup("SetCostume").(*types.Func)
		if !ok {
			t.Fatal("SetCostume function not found")
		}
		apiScopeBindings = schema.APIScopeBindings(setCostumeFn)
		if len(apiScopeBindings) != 1 || apiScopeBindings[0].TargetParam != 1 || apiScopeBindings[0].Source.Receiver || apiScopeBindings[0].Source.Param != 0 {
			t.Fatalf("unexpected bindings for SetCostume: %#v", apiScopeBindings)
		}
	})

	t.Run("ValidInterfaceMethodSpecs", func(t *testing.T) {
		for _, tc := range []struct {
			name     string
			filename string
		}{
			{name: "XGo", filename: "spx.xgo"},
			{name: "Go", filename: "spx.go"},
		} {
			t.Run(tc.name, func(t *testing.T) {
				pkg, conf := loadTestPackage(t, map[string]string{
					tc.filename: `package spx
...
`,
	...
	t.Run("ValidGenericReceiverMethod", func(t *testing.T) {
		pkg, conf := loadTestPackage(t, map[string]string{
			"spx.go": `package spx
...
//
//xgo:class:resource-api-scope-binding param.1 param.0
func (c *Collection[T]) SetCostume(sprite SpriteName, costume SpriteCostumeName) {}
`,
		})
		...
		collectionObj, ok := schema.Package.Scope().Lookup("Collection").(*types.TypeName)
		if !ok {
			t.Fatal("Collection type not found")
		}
		collection, ok := collectionObj.Type().(*types.Named)
		if !ok {
			t.Fatalf("unexpected Collection type: %T", collectionObj.Type())
		}
		setCostume := lookupMethod(t, collection, "SetCostume")
		apiScopeBindings := schema.APIScopeBindings(setCostume)
		if len(apiScopeBindings) != 1 || apiScopeBindings[0].TargetParam != 1 || apiScopeBindings[0].Source.Receiver || apiScopeBindings[0].Source.Param != 0 {
			t.Fatalf("unexpected bindings for Collection.SetCostume: %#v", apiScopeBindings)
		}
	})
...
`,
		...
		_, err := loadTestResourceSchema(pkg, conf)
		if !errorContains(err, `resource kind "sprite" has more than one canonical type`) {
			t.Fatalf("expected duplicate canonical type error, got %v", err)
		}
		if !errorContains(err, "previous at") {
...
`,
		...
		_, err := loadTestResourceSchema(pkg, conf)
		if !errorContains(err, "must declare one top-level resource kind") {
...
`,
		...
		if errorContains(err, "induces a cycle") {
			t.Fatalf("unexpected cycle error for invalid bindings: %v", err)
		}
		if !errorContains(err, "invalid resource-api-scope-binding source param.1 for target param.0") {
...
`,
		...
		if !errorContains(err, "duplicate resource-api-scope-binding target param.0") {
...
//xgo:class:resource orphan.child
type OrphanName string
...
`,
		...
		if !strings.Contains(errs[0].Msg, "undeclared direct parent kind") {
			t.Fatalf("unexpected first error: %v", errs[0])
		}
		for i := 1; i < len(errs); i++ {
			if errs.Less(i, i-1) {
				t.Fatalf("errors are not sorted: %v", errs)
			}
		}
...
func loadTestResourceSchema(pkg *xast.Package, conf *outline.Config) (*ResourceSchema, error) {
	return LoadResourceSchema(pkg, &modfile.Project{PkgPaths: []string{"example.com/spx"}}, conf)
}
...
func errorContains(err error, text string) bool {
	if errs, ok := err.(scanner.ErrorList); ok {
		for _, scanErr := range errs {
			if strings.Contains(scanErr.Msg, text) {
				return true
			}
		}
		return false
