...
func TestResourceSchemaLoaderAddRawAPIScopeBindings(t *testing.T) {
	...
	l := resourceSchemaLoader{
		...
		rawAPIScopeBindings: make(map[*types.Func][]rawAPIScopeBinding),
	}

	l.addRawAPIScopeBindings(fn, []rawAPIScopeBinding{
...
func TestResourceSchemaLoaderValidateKinds(t *testing.T) {
	...
	kindPos := file.Pos(12)
	kind := &ResourceKind{Name: "sprite.costume", pos: kindPos}
	l := resourceSchemaLoader{
		fset: fset,
		schema: &ResourceSchema{
			Kinds: []*ResourceKind{kind},
...
func TestIsStringBased(t *testing.T) {
	t.Run("DefinedString", func(t *testing.T) {
		obj := types.NewTypeName(0, nil, "SpriteName", nil)
		types.NewNamed(obj, types.Typ[types.String], nil)
		if !isStringBased(obj) {
			t.Fatal("expected exported defined string type")
		}
	})
...
func TestIsHandleBearing(t *testing.T) {
	...
	t.Run("Alias", func(t *testing.T) {
		targetObj := types.NewTypeName(0, nil, "SpriteImpl", nil)
		target := types.NewNamed(targetObj, types.NewStruct(nil, nil), nil)
		aliasObj := types.NewTypeName(0, nil, "SpriteAlias", nil)
		types.NewAlias(aliasObj, target)
		if isHandleBearing(aliasObj) {
