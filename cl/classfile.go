...
// hasWorkClassType reports whether name is generated by a work class with all
// requested feats. If feats is zero, any work class matches.
func (p *classProject) hasWorkClassType(name string, feats workClassFeat) bool {
	for _, work := range p.works {
		if feats != 0 && work.feats&feats != feats {
			continue
		}
		if slices.Contains(work.types, name) {
			return true
		}
	}
	return false
