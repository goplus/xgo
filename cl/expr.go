...
func compileIdent(ctx *blockCtx, ident *ast.Ident, flags int) (pkg gogen.PkgRef, kind int) {
	...
	if obj := ctx.pkg.Builtin().TryRef(name); obj != nil {
		...
		if (clCommandIdent&flags) != 0 && recv != nil && xgoMember(cb, recv, "XGo_Exec", ident) == nil {
			kind = objXGoExec
			return
		}
		// for support XGo_Env, see TestSpxXGoEnv
		if (clIdentInStringLitEx&flags) != 0 && recv != nil && xgoMember(cb, recv, "XGo_Env", ident) == nil {
			kind = objXGoEnv
			return
		}
...
func compileEnvExpr(ctx *blockCtx, v *ast.EnvExpr) {
	...
	if ctx.isClass { // in an XGo class file
		if recv := classRecv(cb); recv != nil {
			if xgoMember(cb, recv, "XGo_Env", v) == nil {
				name := v.Name
...
func tryXGoExec(cb *gogen.CodeBuilder, ifn *ast.Ident) bool {
	if recv := classRecv(cb); recv != nil {
		cb.InternalStack().PopN(1)
		if xgoMember(cb, recv, "XGo_Exec", ifn) == nil {
			cb.Val(ifn.Name, ifn)
			return true
		}
	}
