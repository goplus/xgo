...
func TestProviderArgsRun(t *testing.T) {
	...
	want := []string{
		...
		"--module-root=/project",
		"--provider-package=example.test/framework/cmd/provider",
...
func TestProviderArgsBuildSelected(t *testing.T) {
	rt := testRuntime()
	rt.Origin.Replace = nil
	rt.Origin.Selected.Dir = "/framework"
	rt.Origin.Selected.GoMod = "/framework/go.mod"
	got, err := providerArgs(rt, actionBuild, BuildPolicy{}, "/tmp/stage/game", "/out/game", nil)
	if err != nil {
		t.Fatal(err)
	}
	joined := strings.Join(got, "\n")
...
func TestProviderArgsInvalid(t *testing.T) {
	rt := testRuntime()
	rt.Protocol = "v2"
	if _, err := providerArgs(rt, actionRun, BuildPolicy{}, "", "", nil); err == nil {
		t.Fatal("unsupported protocol succeeded")
	}
	rt.Protocol = "v1"
	if _, err := providerArgs(rt, actionBuild, BuildPolicy{}, "", "", nil); err == nil {
...
func TestValidateArgv(t *testing.T) {
	...
	if err := validateArgv("provider", []string{big}, nil); !errors.Is(err, ErrRuntimeArgvTooLarge) {
