...
// runtimeProviderCapability is the XGo capability level claimed by unversioned
// development builds. Released binaries continue to use env.Version().
const runtimeProviderCapability = "1.8.0"
...
func checkRequiredXGo(required, current string) error {
	...
	if !ok {
		return fmt.Errorf("invalid required XGo version %q (xgo build %s)", required, describeRuntimeProviderVersion(current))
	}
...
func comparableRuntimeProviderVersion(version string) (string, bool) {
	if isDevelopmentVersion(version) {
		return normalizeXGoVersion(runtimeProviderCapability)
	}
	return comparableCurrentVersion(version)
}

func describeRuntimeProviderVersion(version string) string {
	if isDevelopmentVersion(version) {
		return fmt.Sprintf("%s (runtime-provider capability %s)", version, runtimeProviderCapability)
	}
	return version
}
...
func isDevelopmentVersion(version string) bool {
	...
	return version == "(devel)" || strings.HasSuffix(version, " devel")
}
...
func comparableCurrentVersion(version string) (string, bool) {
	...
	// development builds are handled by comparableRuntimeProviderVersion.
