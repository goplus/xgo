...
type runtimeSignalCause struct {
	signal syscall.Signal
}

func (c runtimeSignalCause) Error() string {
	return fmt.Sprintf("runtime interrupted by %s", c.signal)
}
	...
	go func() {
		...
		for {
			...
			case received := <-b.signals:
				...
				if b.signal == 0 {
					b.signal = unixSignal
					b.cancel(runtimeSignalCause{signal: unixSignal})
				}
				b.mu.Unlock()
			case <-b.done:
				return
