...
func (r *Resolver) resolveTargetGraph(ctx context.Context, projectDir string, graph *effectiveGraph, policy GraphPolicy, recursive bool) (*effectiveGraph, error) {
	if graph != nil {
		...
		hasRuntime, err := overlayRuntimeProjectMatch(projectDir, graph, recursive)
		if err != nil {
			return nil, err
		}
		if !hasRuntime {
	...
	// introduce class markers or runtime metadata absent from the physical tree.
	if overlay := policy.Overlay; overlay != "" {
		...
		hasRuntime, err := overlayRuntimeProjectMatch(projectDir, overlayGraph, recursive)
		if err != nil {
			return nil, err
		}
		if !hasRuntime {
	...
	if vendor {
		hasRuntime, err := r.probeVendorProject(projectDir, preflightModule, recursive)
		if err != nil {
			return nil, err
		}
		if !hasRuntime {
