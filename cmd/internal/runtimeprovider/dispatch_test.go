...
func TestDispatchRejectsMixedTargetsBeforeProvider(t *testing.T) {
	if testing.Short() {
		t.Skip("invokes the host Go command")
	}
	fixture := newRuntimeFixture(t)
	...
	if _, err := os.Stat(marker); !os.IsNotExist(err) {
		t.Fatalf("provider started before target validation: %v", err)
	}
}

func setFixtureRequiredXGo(t *testing.T, fixture runtimeFixture, version string) {
	t.Helper()
