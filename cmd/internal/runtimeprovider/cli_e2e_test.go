...
func TestCLIEndToEnd(t *testing.T) {
	if testing.Short() {
		t.Skip("builds the XGo command and fake provider")
	}
	buildGoWork := os.Getenv("GOWORK")
	modDir := protocolModuleDir(t, buildGoWork)
	fixture := newRuntimeFixture(t)
	configureSharedProtocolProvider(t, fixture, modDir)
	...
	t.Setenv("FAKE_PROVIDER_EXPECT_NO_PACK", "1")
	...
	multi.Env = append(multi.Env, "GOBIN="+multiBin, "FAKE_PROVIDER_MARKER="+marker)
	output, err = multi.CombinedOutput()
	if err == nil || !strings.Contains(string(output), "exactly one target") {
		t.Fatalf("multi runtime install = %v\n%s", err, output)
	}
	if _, err := os.Stat(marker); !os.IsNotExist(err) {
		t.Fatalf("provider ran before multi-target rejection: %v", err)
	}
...
func configureSharedProtocolProvider(t *testing.T, fixture runtimeFixture, modDir string) {
	t.Helper()
	mustWriteFile(t, filepath.Join(fixture.framework, "cmd", "provider", "main.go"), protocolFakeProviderSource)
	...
	if output, err := download.CombinedOutput(); err != nil {
		t.Fatalf("prepare shared-protocol provider graph: %v\n%s", err, output)
	}
	goMod, err = os.ReadFile(goModPath)
	if err != nil {
		t.Fatal(err)
	...
	if _, err := os.Stat(filepath.Join(fixture.app, "runtimeprotocol_tools.go")); !os.IsNotExist(err) {
		t.Fatalf("fixture contains a test-only tools file: %v", err)
	}
}

const protocolFakeProviderSource = `package main
...
func main() {
	...
	request, err := runtimeprotocol.Parse(os.Args[1:])
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(90)
	}
	if os.Getenv("FAKE_PROVIDER_EXPECT_NO_PACK") != "" && request.Project.Pack != nil {
		os.Exit(94)
	}
	switch request.Action {
	case runtimeprotocol.ActionRun:
		fmt.Printf("run-args=%s\n", strings.Join(request.ApplicationArgs, "|"))
	case runtimeprotocol.ActionBuild:
...
func assertNoAutogen(t *testing.T, dir string) {
	...
	if _, err := os.Stat(filepath.Join(dir, "xgo_autogen.go")); !os.IsNotExist(err) {
		t.Fatalf("runtime path touched xgo_autogen.go: %v", err)
	}
}
