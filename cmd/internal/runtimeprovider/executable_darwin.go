...
func validateHostExecutable(input *os.File, path string) error {
	...
	if err != nil {
		return fmt.Errorf("runtime provider output is not a Darwin executable: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("resolve runtime provider output %q: %w", path, err)
	}
	...
	if info, err := os.Lstat(currentPath); err != nil || !os.SameFile(identity, info) {
		return fmt.Errorf("runtime provider output changed before signature validation")
	}
	cmd := exec.Command("/usr/bin/codesign", "--verify", "--strict", currentPath)
	if output, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("runtime provider output has no valid Darwin signature: %w: %s", err, strings.TrimSpace(string(output)))
	}
	if info, err := os.Lstat(currentPath); err != nil || !os.SameFile(identity, info) {
		return fmt.Errorf("runtime provider output changed during signature validation")
	}
	return nil
}
