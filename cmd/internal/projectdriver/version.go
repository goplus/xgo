...
// driverCapability is the XGo capability level claimed by unversioned
// development builds. Released binaries continue to use env.Version().
const driverCapability = "1.8.0"
...
func comparableDriverVersion(version string) (string, bool) {
	if isDevelopmentVersion(version) {
		return normalizeXGoVersion(driverCapability)
	}
...
func describeDriverVersion(version string) string {
	if isDevelopmentVersion(version) {
		return fmt.Sprintf("%s (driver capability %s)", version, driverCapability)
	}
	return version
}
