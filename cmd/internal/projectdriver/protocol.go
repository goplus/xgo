...
func driverArgs(drv *Driver, act action, policy BuildPolicy, output, finalOutput string, appArgs []string) ([]string, error) {
	if drv.Protocol != protocolV1 {
		return nil, fmt.Errorf("unsupported driver protocol %q", drv.Protocol)
	}
	var pack *driverprotocol.Pack
	if drv.PackDir != "" || drv.PackIndex != "" {
		if drv.PackDir == "" || drv.PackIndex == "" {
			return nil, fmt.Errorf("driver pack metadata must contain both directory and index")
		}
	...
	default:
		return nil, fmt.Errorf("unsupported driver action %q", act)
	}
