...
func driverGuard(projectDir, driverPackage string) string {
	return sha256Bytes([]byte(projectDir + "\x00" + driverPackage))
}
...
func driverProjectMatches(projects []*modfile.Project, name string) bool {
	...
	for _, project := range projects {
		if project != nil && project.Driver != nil && project.IsProj(ext, name) {
			return true
		}
	}
	return false
...
func findProjectFile(dir string, module *xgomod.Module) (string, *xgomod.ProjectInfo, int, error) {
	...
	for _, entry := range entries {
		...
		count++
		if classInfo.Project.Driver != nil {
...
func patternContainsDriverProject(root string, module *xgomod.Module) (bool, error) {
	return walkDriverPattern(root, func(dir string) (bool, error) {
		...
		return info != nil && info.Project != nil && info.Project.Driver != nil, err
	})
}
...
func declaringMetadata(origin ResolvedModule, snapshot xgomod.FileIdentity) (fileIdentity, error) {
	...
	if len(snapshot.SHA256) != sha256.Size*2 {
		return fileIdentity{}, fmt.Errorf("driver origin %q has an invalid declaring metadata digest", origin.Selected.Path)
	}
