...
func TryRun(ctx context.Context, cwd string, target xgoprojs.Proj, flags, appArgs []string, streams Streams) (DispatchResult, error) {
	...
	return dispatchOne(ctx, cwd, target, flags, "run", func(ctx context.Context, resolver *Resolver, drv *Driver) (ProcessStatus, string, error) {
...
func TryBuild(ctx context.Context, cwd string, target xgoprojs.Proj, flags []string, output string, streams Streams) (DispatchResult, error) {
	return dispatchOne(ctx, cwd, target, flags, "build", func(ctx context.Context, resolver *Resolver, drv *Driver) (ProcessStatus, string, error) {
...
func dispatchOne(ctx context.Context, cwd string, target xgoprojs.Proj, flags []string, action string, run func(context.Context, *Resolver, *Driver) (ProcessStatus, string, error)) (DispatchResult, error) {
	if target == nil {
		return DispatchResult{}, fmt.Errorf("driver %s requires a non-nil target", action)
	}
	resolver, ctx, err := newDispatchResolver(ctx, cwd, flags)
	if err != nil {
		return DispatchResult{}, err
...
func finishDispatch(boundary *driverSignalBoundary, result DispatchResult, status ProcessStatus, err error) (DispatchResult, error) {
...
func resolveTarget(ctx context.Context, resolver *Resolver, target xgoprojs.Proj) (*Driver, bool, error) {
	if target == nil {
		return nil, false, fmt.Errorf("driver dispatch requires a non-nil target")
	}
