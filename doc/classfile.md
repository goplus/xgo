...
The compiler reads this signature to know how many kinds of work classes exist and how to group instances when calling the method.
...
- `main.spx` is the project classfile; its class embeds `spx.Game`. Top-level code becomes `MainEntry`.
- All other `*.spx` files are work classfiles; each embeds `spx.SpriteImpl`. The `-embed` flag causes each work class instance to be declared as a named field on the project class.
...
In `MyAircraft.spx`, firing is trivial — just clone the bullet sprite:

```go
onStart => {
    for {
        wait 0.1
        Bullet.clone          // create a new Bullet instance
        setXYpos mouseX, mouseY
    }
}
```

`Bullet.spx` handles all the rest:
...
func (this *Game) MainEntry() { /* user code */ }
...
func (this *Game) Main() {
    ...
    this.Bullet, this.Enemy, this.MyAircraft = bullet, enemy, myAircraft
...
func (this *Bullet) Main() { /* user code */ }

func main() { new(Game).Main() }
```
...
- `*_app.gox` is the project classfile; the class embeds `xcmd.App`.
- `*_cmd.gox` files are work classfiles; each embeds `xcmd.Command`. The `-prefix=Cmd_` flag prepends `Cmd_` to every generated type name, so `run_cmd.gox` produces type `Cmd_run` rather than `run`, avoiding collisions with common identifiers.
...
#### Custom `Main` signature via `ProtocolType`

`xcmd.Command` implements `iCommandProto`:
...
This tells the compiler every work class `Main` must accept a `string` argument. The compiler also inserts a base class call automatically:

```go
func (this *Cmd_run) Main(_xgo_arg0 string) {
    this.Command.Main(_xgo_arg0) // auto-inserted by compiler
...
Every handler's `Main` receives a `*Context` giving direct access to the HTTP request and response.
...
```go
type MyServer struct { server.MCPApp }

func (this *MyServer) MainEntry() { /* user code */ }
...
func (this *Search) Main() { /* user code */ }
```
...
Because the MCP protocol distinguishes resources, tools, and prompts at runtime, the framework receives them as separate typed slices rather than a single variadic. The compiler groups each work class instance by its declared `ProtocolType` and passes them into the corresponding slice argument.
